...
const V29_KEY='gplus_pism_official_session_v29';
...
function proxy(url,kind){return `/api/${kind==='v29'?'pism':'enem'}-pdf?url=${encodeURIComponent(url)}`}
function session(kind){return parse(localStorage.getItem(kind==='v27'?V27_KEY:kind==='v29'?V29_KEY:V28_KEY),null)}
...
function readerMarkup(source,kind){const pism=kind==='v29',provider=pism?'UFJF/COPESE':'INEP';return `<section class="v32-reader" data-source="${source}"><div class="v32-reader-top"><div><span class="v32-kicker">${pism?'PISM OFICIAL':'CADERNO OFICIAL'} · LEITOR GABARITO+</span><b class="v32-question-label">Questão</b></div><div class="v32-page-actions"><button type="button" class="v32-page-btn" data-v32-prev aria-label="Página anterior">‹</button><span class="v32-page-label" aria-live="polite">Carregando…</span><button type="button" class="v32-page-btn" data-v32-next aria-label="Próxima página">›</button></div></div><div class="v32-stage"><div class="v32-loading"><span></span><b>Preparando caderno oficial…</b><small>Sem barra de PDF e sem sair do Gabarito+.</small></div><canvas class="v32-canvas" hidden></canvas></div><div class="v32-reader-foot"><span>Conteúdo oficial da ${provider} renderizado dentro do aplicativo.</span><a href="${source}" target="_blank" rel="noopener">Abrir original</a></div></section>`}
...
function showFriendlyFailure(state){
 ...
 const provider=state.kind==='v29'?'UFJF':'Inep';
 const retryClass=state.kind==='v29'?'btn-pism':'btn-primary';
 loading.innerHTML=`<b>Caderno oficial temporariamente indisponível.</b><small>Seu cartão-resposta e seu progresso continuam salvos. Tente novamente ou abra a fonte oficial.</small><div class="v32-failure-actions"><button type="button" class="btn ${retryClass}" data-v32-retry>Tentar novamente</button><a class="btn btn-secondary" href="${state.source}" target="_blank" rel="noopener">Abrir na ${provider}</a></div>`;
...
async function mount(iframe,kind){
 ...
 const paper=iframe.closest(kind==='v27'?'.v27-paper':kind==='v29'?'.v29-paper':'.v28-paper');if(!paper)return;
 if(readers.has(paper)){await syncQuestion(readers.get(paper));return}
 const holder=document.createElement('div');holder.innerHTML=readerMarkup(source,kind);const host=holder.firstElementChild;iframe.replaceWith(host);paper.classList.add('v32-native-document');
 ...
 try{
  const pdfjs=await loadPdfJs(),task=pdfjs.getDocument({url:proxy(source,kind),withCredentials:false,disableRange:true,disableStream:true,disableAutoFetch:true});state.doc=await task.promise;
...
async function enhance(){
 ...
 const v29=$('#v29PismOfficialRunner .v29-paper iframe');if(v29)await mount(v29,'v29');
 for(const state of readers.values())if(state.host.isConnected)syncQuestion(state);
 window.GABARITO_APP=window.GABARITO_APP||{};window.GABARITO_APP.enemDocument=VERSION;window.GABARITO_APP.officialDocument=VERSION;
}
function activeReaderOrIframe(){if($('#v27OfficialRunner .v27-paper iframe,#v28Runner .v28-paper iframe,#v29PismOfficialRunner .v29-paper iframe'))return true;for(const state of readers.values())if(state.host.isConnected)return true;return false}
...
function boot(){
 ...
 setTimeout(enhance,400);window.GABARITO_ENEM_DOCUMENT={version:VERSION,enhance};window.GABARITO_OFFICIAL_DOCUMENT={version:VERSION,enhance}
}
