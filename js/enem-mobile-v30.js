...
function historical(){return parse(localStorage.getItem(HISTORY_KEY),null)}
function isOfficialKind(kind){return kind==='official'||kind==='history'}
function runner(kind){return kind==='authorial'?$('#v24EnemRunner'):kind==='history'?$('#v28Runner'):$('#v27OfficialRunner')}
function sheet(kind){return kind==='history'?'#v28Sheet':'#v27Sheet'}
function visible(el){return Boolean(el&&el.isConnected&&!el.hidden&&getComputedStyle(el).display!=='none')}
function mode(){
 if(!isMobile())return null;
 const hist=$('#v28Runner');
 if(visible(hist)&&$('.v28-workspace',hist)&&historical()?.status==='active')return'history';
...
function state(kind){return kind==='official'?official():kind==='history'?historical():authorial()}
function currentNumber(kind,s){return isOfficialKind(kind)?Number(s.current||1):Number(s.index||0)+1}
function selectedLetter(kind,s){
 const q=currentNumber(kind,s);
 if(isOfficialKind(kind))return String(s.answers?.[String(q)]||'');
 const id=s.ids?.[q-1],value=s.selections?.[id];
 return Number.isInteger(value)?String.fromCharCode(65+value):'';
}
function answeredCount(kind,s){return isOfficialKind(kind)?Object.keys(s.answers||{}).length:Object.keys(s.selections||{}).length}
...
function goQuestion(kind,q){
 if(isOfficialKind(kind)){
  const selector=kind==='history'?`${sheet(kind)} [data-v28-q="${q}"]`:`${sheet(kind)} [data-q="${q}"]`;
  const button=$(selector);if(button)button.click();
 }else{
  const button=$(`#v24EnemRunner .v24-sheet [data-jump="${q-1}"]`);if(button)button.click();
 }
 afterNav(kind);
}
function chooseOfficial(kind,letter){
 const selector=kind==='history'?`${sheet(kind)} [data-v28-letter="${letter}"]`:`${sheet(kind)} [data-letter="${letter}"]`;
 const button=$(selector);if(button)button.click();
 enhance();
}
function previous(kind){
 ...
 const q=Math.max(s.day===1?1:91,Number(s.current)-1),selector=kind==='history'?`${sheet(kind)} [data-v28-q="${q}"]`:`${sheet(kind)} [data-q="${q}"]`;$(selector)?.click();enhance();
}
function next(kind){
 const s=state(kind);if(!s)return;
 if(kind==='authorial'){trigger('[data-next]',kind);setTimeout(()=>afterNav(kind),20);return}
 const q=Math.min(s.day===1?90:180,Number(s.current)+1),selector=kind==='history'?`${sheet(kind)} [data-v28-q="${q}"]`:`${sheet(kind)} [data-q="${q}"]`;$(selector)?.click();enhance();
}
function toggleMark(){trigger('[data-mark]','authorial');enhance()}
function openPanel(){
 const kind=mode(),s=state(kind);if(!kind||!s)return;
 lastFocus=document.activeElement;
 const q=currentNumber(kind,s),relative=isOfficialKind(kind)&&s.day===2?q-90:q;
...
function panelQuestions(kind,s){
 const absoluteStart=isOfficialKind(kind)&&s.day===2?91:1;
 ...
 for(let q=start;q<=end;q++){
  ...
  let answered=false,marked=false,letter='';
   if(isOfficialKind(kind)){letter=String(s.answers?.[String(q)]||'');answered=Boolean(letter)}
...
function renderPanel(kind,s){
 ...
 $$('[data-v30-letter]',panel).forEach(b=>b.addEventListener('click',()=>chooseOfficial(kind,b.dataset.v30Letter)));
 ...
 $('[data-v30-exit]',panel)?.addEventListener('click',()=>{closePanel();trigger(kind==='history'?'[data-v28-exit]':kind==='official'?'[data-v27-exit]':'[data-exit]',kind)});
 $('[data-v30-finish]',panel)?.addEventListener('click',()=>{closePanel();trigger(kind==='history'?'[data-v28-finish]':kind==='official'?'[data-v27-finish]':'[data-finish]',kind)});
}
function renderDock(kind,s){
 ...
 $$('[data-v30-letter]',dock).forEach(b=>b.addEventListener('click',()=>chooseOfficial(kind,b.dataset.v30Letter)));
}
function enhance(){
 const kind=mode();
