...
async function loadLocalBank(){show('Abrindo conteúdo local…');try{await loadScript('data/editorial-exclusions.js',3000)}catch(e){console.warn('[Gabarito+] Lista editorial local indisponível:',e.message)}await loadScript('data/enem-questions.js',5000);await loadScript('data/pism-questions.js',5000);await loadScript('data/pism-discursives.js',5000);try{await loadScript('data/pism-discursives-v25.js',3000)}catch(e){console.warn('[Gabarito+] Expansão discursiva do PISM indisponível:',e.message)}applyEditorialExclusions();window.GABARITO_APP.bankSource='local-reviewed';window.GABARITO_APP.cloudStatus=configured?'account-pending':'offline'}
