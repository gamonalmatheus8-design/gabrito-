...
window.closeV5Account=function(){$('v5AccountModal')?.classList.remove('open');$('v5AccountModal')?.setAttribute('aria-hidden','true')}
window.v5AuthTab=function(tab){document.querySelectorAll('[data-v5tab]').forEach(b=>b.classList.toggle('active',b.dataset.v5tab===tab));$('v5LoginForm')?.classList.toggle('hidden',tab!=='login');$('v5RegisterForm')?.classList.toggle('hidden',tab!=='register')}
function requireClient(){if(!client)throw new Error('Supabase ainda não está conectado. Preencha js/supabase-config.js.')}
async function loadCloud(){requireClient();if(!S.user)return;setStatus('Verificando…','syncing');const {data,error}=await client.from('progress_state').select('snapshot,revision,updated_at').eq('user_id',S.user.id).maybeSingle();if(error)throw error;const local=snapshot();S.revision=Number(data?.revision||0);if(!data){await upload(local);S.resolved=true;return}if(!meaningful(local)){applySnapshot(data.snapshot||{});S.lastHash=hash(snapshot());S.resolved=true;setStatus('Sincronizado','ok');setTimeout(()=>location.reload(),220);return}const last=localStorage.getItem(SYNC_KEY);if(last&&data.updated_at&&new Date(data.updated_at)<=new Date(last)){await upload(local);S.resolved=true;return}S.resolved=false;setStatus('Escolher versão','warn');$('v5Conflict')?.classList.remove('hidden')}
async function upload(s=snapshot()){
  if(!client||!S.user||S.busy)return;S.busy=true;setStatus('Enviando…','syncing');
  try{const {data,error}=await client.rpc('save_progress',{p_expected_revision:S.revision,p_snapshot:s});if(error)throw error;const r=Array.isArray(data)?data[0]:data;if(r?.conflict){S.resolved=false;setStatus('Conflito','warn');$('v5Conflict')?.classList.remove('hidden');return}S.revision=Number(r?.revision||S.revision+1);S.lastHash=hash(s);localStorage.setItem(SYNC_KEY,r?.updated_at||new Date().toISOString());S.resolved=true;setStatus('Sincronizado','ok')}catch(e){console.error(e);setStatus(navigator.onLine?'Erro de sync':'Offline','error')}finally{S.busy=false}
}
window.v5UseCloud=async function(){requireClient();const {data,error}=await client.from('progress_state').select('snapshot,revision,updated_at').eq('user_id',S.user.id).maybeSingle();if(error)throw error;if(data){applySnapshot(data.snapshot||{});S.revision=Number(data.revision||0);S.lastHash=hash(snapshot());S.resolved=true;$('v5Conflict')?.classList.add('hidden');setStatus('Sincronizado','ok');setTimeout(()=>location.reload(),180)}}
window.v5UseDevice=async function(){$('v5Conflict')?.classList.add('hidden');const {data}=await client.from('progress_state').select('revision').eq('user_id',S.user.id).maybeSingle();S.revision=Number(data?.revision||0);await upload(snapshot())}
window.v5SyncNow=async function(){if(!S.user)return openV5Account();await upload(snapshot())}
...
async function logAttempt(id,index){if(!client||!S.user)return;try{const q=QUESTIONS.find(x=>String(x.id)===String(id));if(!q)return;await client.from('question_attempts').insert({user_id:S.user.id,question_id:String(id),exam:q.exam,module:q.module||null,subject:q.subject,topic:q.topic,correct:Number(index)===Number(q.answer)})}catch(e){console.warn('[Gabarito+] analytics:',e.message)}}
function instrument(){const base=window.answerQuestion;if(base&&!base.__v7){const wrapped=function(id,index){const result=base(id,index);queueMicrotask(()=>logAttempt(id,index));return result};wrapped.__v7=true;window.answerQuestion=wrapped}}

function instrumentEssay(){
  const base=window.correctEssayWithAI;if(!base||base.__v7)return;
  const wrapped=async function(){
    if(!client||!S.user)return base();
    const text=document.getElementById('essayText').value.trim(),theme=document.getElementById('essayTheme').textContent.trim(),title=document.getElementById('essayTitle').value.trim(),host=document.getElementById('aiCorrectionResult'),status=document.getElementById('aiCorrectionStatus'),btn=document.getElementById('aiCorrectBtn');
    if(text.split(/\s+/).filter(Boolean).length<80){toast('Escreva uma redação mais completa antes da correção por IA.');return}
    if(btn?.disabled)return;status.textContent='Analisando C1–C5 com IA…';if(btn){btn.disabled=true;btn.textContent='Corrigindo…'};host.innerHTML='<div class="notice">Correção em andamento. O resultado é uma estimativa pedagógica.</div>';
    try{const {data,error}=await client.functions.invoke('correct-essay',{body:{text,theme,title}});if(error)throw error;if(data?.error)throw new Error(data.error);renderAICorrection(data);status.textContent='Correção concluída · estimativa pedagógica por IA'}catch(e){host.innerHTML=`<div class="notice"><strong>Não foi possível concluir a correção.</strong><br>${esc(e.message||e)}</div>`;status.textContent='Correção não concluída'}finally{if(btn){btn.disabled=false;btn.innerHTML='<i data-lucide="sparkles" class="icon"></i>Corrigir com IA';if(window.lucide)lucide.createIcons()}}
  };wrapped.__v7=true;window.correctEssayWithAI=wrapped;
}
async function boot(){
