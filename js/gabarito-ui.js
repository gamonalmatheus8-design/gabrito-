...
function loadStabilityLayer(){loadStyle('/assets/v9-stability.css','v9-stability-style');if(!window.GabaritoV92&&!document.querySelector('script[data-gplus-layer="v9-stability-script"]'))loadScript('/js/v9-stability.js','v9-stability-script',null,()=>console.warn('[Gabarito+] A camada de estabilidade da V9 não pôde ser carregada.'));if(!window.GabaritoV92QuestionSpeed&&!document.querySelector('script[data-gplus-layer="v9-question-speed-script"]'))loadScript('/js/v9-question-speed.js','v9-question-speed-script',null,()=>console.warn('[Gabarito+] O caminho rápido de questões não pôde ser carregado.'))}
...
function initMore(){polishPublicShell();const nav=document.getElementById('v42MoreNav'),btn=document.getElementById('v42MoreToggle');if(nav&&btn){const open=nav.classList.contains('open');btn.setAttribute('aria-expanded',String(open));nav.hidden=!open;nav.style.display=open?'block':'none'}loadV9Layer()}
window.toggleMoreNav=function(force){polishPublicShell();const nav=document.getElementById('v42MoreNav'),btn=document.getElementById('v42MoreToggle');if(!nav||!btn)return;const current=btn.getAttribute('aria-expanded')==='true'||nav.classList.contains('open'),open=typeof force==='boolean'?force:!current;btn.setAttribute('aria-expanded',String(open));btn.classList.toggle('open',open);nav.classList.toggle('open',open);nav.hidden=!open;nav.style.display=open?'block':'none';if(window.lucide)window.lucide.createIcons()};
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initMore,{once:true});else initMore();
setTimeout(()=>{polishPublicShell();const source=document.getElementById('v7BankSource');if(source){const s=window.GABARITO_APP?.bankSource||'local';source.textContent=String(s).startsWith('supabase')?'Conteúdo atualizado':'Modo offline';source.title=String(s).startsWith('supabase')?'Seu banco de estudos está atualizado.':'O conteúdo essencial continua disponível neste dispositivo.';source.dataset.gplusReady='true'}loadV9Layer()},0);
})();
