...
'use strict';
const VERSION='3.7.0';
const RECOVERY='20260908-official-bank-v2';
...
async function loadPismCore(){await Promise.all([loadStyle('assets/pism-history-v29.css'),loadStyle('assets/enem-document-v32.css'),loadScript('data/pism-official-catalog-v29.js')]);await loadScript('js/pism-history-v29.js');await loadScript('js/pism-direct-sync-v37.js');if(window.GABARITO_PISM_DIRECT_SYNC?.version!=='3.7.0')throw new Error('Sincronização do caderno PISM não carregou.')}
async function loadEnhancements(){if(enhancePromise)return enhancePromise;enhancePromise=(async()=>{try{await loadStyle('assets/enem-mobile-v30.css');await loadScript('js/enem-mobile-v30.js');await loadScript('js/enem-mobile-official-bridge-v37.js');window.GABARITO_APP.simulatorEnhancementsReady=true}catch(e){console.warn('[Gabarito+] Melhorias móveis dos simulados indisponíveis:',e.message);window.GABARITO_APP.simulatorEnhancementError=e.message}})();return enhancePromise}
async function ensureLoaded(force=false){if(ready&&!force)return true;if(corePromise&&!force)return corePromise;failed=null;setStatus('loading','Carregando provas oficiais e leitores sincronizados.');corePromise=(async()=>{try{await Promise.all([loadEnemCore(),loadPismCore()]);await new Promise(r=>setTimeout(r,120));ready=true;window.GABARITO_APP.simulatorsLazyReady=true;window.GABARITO_APP.simulatorsLoadedAt=Date.now();window.GABARITO_APP.simulatorsSyncRequired=true;window.GABARITO_APP.simulatorReader='official-direct-sync';window.GABARITO_APP.enemSimulatorReader='direct-sync-v36';window.GABARITO_APP.pismSimulatorReader='direct-sync-v37';setStatus('ready');try{window.GABARITO_ENEM_HISTORY?.enhance?.()}catch{}try{window.GABARITO_PISM_OFFICIAL?.enhance?.()}catch{}loadEnhancements().catch(()=>{});return true}catch(e){failed=e;corePromise=null;window.GABARITO_APP.simulatorsLazyError=e.message;setStatus('error',e.message);throw e}})();return corePromise}
function wrapGo(){if(typeof window.go!=='function'||window.go.__gplusLazySimulators)return;const base=window.go;const wrapped=function(page,...args){const result=base.call(this,page,...args);if(page==='mocks')ensureLoaded().catch(()=>{});return result};Object.assign(wrapped,base);wrapped.__gplusLazySimulators=true;window.go=wrapped}
function init(){window.GABARITO_APP=window.GABARITO_APP||{};window.GABARITO_APP.lazySimulators=VERSION;wrapGo();if($('#page-mocks')?.classList.contains('active')||localStorage.getItem('study_v2_page')==='mocks')ensureLoaded().catch(()=>{});const observer=new MutationObserver(()=>{if(!window.go?.__gplusLazySimulators)wrapGo()});observer.observe(document.documentElement,{subtree:true,childList:true});window.GABARITO_SIMULATORS_LAZY={version:VERSION,ensureLoaded,get ready(){return ready},get failed(){return failed}}}
