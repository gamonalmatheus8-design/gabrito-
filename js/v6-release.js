(function(){
'use strict';
const VERSION='2.0.0',SW_VERSION='3.6.0',SW_RECOVERY='20260908-enem-stable1';let installPrompt=null;
...
if('serviceWorker' in navigator){const reg=()=>navigator.serviceWorker.register(`/service-worker.js?v=${SW_VERSION}&r=${SW_RECOVERY}`,{scope:'/',updateViaCache:'none'}).then(async r=>{await r.update().catch(()=>{});if(r.waiting)r.waiting.postMessage?.({type:'SKIP_WAITING'})}).catch(()=>{});if(document.readyState==='complete')reg();else window.addEventListener('load',reg,{once:true})}
