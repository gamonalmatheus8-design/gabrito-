...
if('serviceWorker' in navigator){const reg=()=>navigator.serviceWorker.register(`/service-worker.js?v=${SW_VERSION}&r=${SW_RECOVERY}`,{scope:'/',updateViaCache:'none'}).then(r=>r.update().catch(()=>{})).catch(()=>{});if(document.readyState==='complete')reg();else window.addEventListener('load',reg,{once:true})}
