...
try{
 ...
 await page.waitForFunction(()=>window.GABARITO_ENEM_DIRECT_SYNC?.version==='3.6.0',{timeout:10000});
 ...
 await page.waitForSelector('#v28Runner .gplus-direct-sync-reader[data-direct-sync-version="3.6.0"]',{timeout:7000});
 ...
 assert.match(await page.locator('#v28Runner .gplus-direct-sync-reader').innerText(),/GABARITO\+.*SINCRONIZADO/is);
 await page.waitForFunction(()=>document.querySelector('#v28Runner .v32-page-label')?.textContent?.includes('Página 2'),{timeout:7000});
 ...
 await page.evaluate(async()=>{const k='gplus_enem_history_exam_v28',s=JSON.parse(localStorage.getItem(k));s.current=2;localStorage.setItem(k,JSON.stringify(s));await window.GABARITO_ENEM_DIRECT_SYNC.enhance()});
