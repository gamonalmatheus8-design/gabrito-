...
try{
 const authorial=await context.newPage(),errors=[];authorial.on('pageerror',e=>errors.push(e.message));
 ...
 await authorial.close();

 const official=await context.newPage(),officialErrors=[];official.on('pageerror',e=>officialErrors.push(e.message));
