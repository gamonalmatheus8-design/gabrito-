...
try{
 ...
 assert.equal(await page.evaluate(()=>window.GABARITO_APP?.lazySimulators),'3.5.0');
 ...
 await page.waitForFunction(()=>window.GABARITO_ENEM_DIRECT_SYNC?.version==='3.5.0',{timeout:7000});
 ...
 assert.equal(await page.evaluate(()=>window.GABARITO_ENEM_DIRECT_SYNC?.version),'3.5.0');
 ...
 assert.ok(after.some(x=>x.includes('enem-direct-sync-v35.js')),'sincronização direta v35 deve estar carregada');
