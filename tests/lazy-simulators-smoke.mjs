...
try{
 ...
 assert.equal(await page.evaluate(()=>window.GABARITO_APP?.lazySimulators),'3.6.0');
 const before=await page.evaluate(()=>performance.getEntriesByType('resource').map(x=>x.name));
 for(const needle of ['enem-official-v27.js','enem-history-v28.js','pism-history-v29.js','enem-direct-sync-v36.js','enem-mobile-v30.js']){
  assert.equal(before.some(x=>x.includes(needle)),false,`${needle} não deve carregar no boot`);
 }
 assert.equal(await page.evaluate(()=>Boolean(window.GABARITO_ENEM_DIRECT_SYNC)),false,'sincronizador não deve rodar no boot global');
 ...
 assert.equal(await page.evaluate(()=>window.GABARITO_APP?.simulatorReader),'direct-sync-v36');
 ...
 assert.ok(after.some(x=>x.includes('enem-direct-sync-v36.js')),'sincronização direta v36 deve carregar com Simulados');
 assert.equal(after.some(x=>x.includes('enem-direct-sync-v35.js')),false,'v35 não deve mais ser carregado');
 assert.equal(after.some(x=>x.includes('enem-document-v32.js')),false,'leitor antigo v32 não deve competir com o v36');
