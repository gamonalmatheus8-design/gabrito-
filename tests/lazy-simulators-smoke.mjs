...
try{
 ...
 assert.equal(await page.evaluate(()=>window.GABARITO_APP?.lazySimulators),'3.4.0');
 const before=await page.evaluate(()=>performance.getEntriesByType('resource').map(x=>x.name));
 for(const needle of ['enem-official-v27.js','enem-history-v28.js','pism-history-v29.js','enem-native-v31.js','enem-document-v32.js','enem-direct-sync-v33.js','enem-mobile-v30.js']){
 ...
 assert.ok(await page.evaluate(()=>Boolean(window.GABARITO_ENEM_DIRECT_SYNC)),'sincronização direta do ENEM é obrigatória no núcleo de Simulados');
 assert.equal(await page.evaluate(()=>window.GABARITO_ENEM_DIRECT_SYNC?.version),'3.4.0');
 ...
 assert.ok(after.some(x=>x.includes('enem-direct-sync-v33.js')),'sincronização direta deve ser baixada junto do núcleo do ENEM');
