...
test('resolvedor decodifica entidades HTML usadas nas páginas reais da COPESE',()=>{const source='https://www2.ufjf.br/copese/pism/teste/';const html=`<a href="m1.pdf">Caderno de Provas do 1&ordm; Dia - M&oacute;dulo I</a><a href="m3.pdf">Caderno de Provas do 1&ordm; Dia - M&oacute;dulo III - ECONOMIA E ADMINISTRA&Ccedil;&Atilde;O</a><a href="g1.pdf">Gabarito das Provas do 1&ordm; Dia - M&oacute;dulo I - ap&oacute;s recurso</a>`;const one=resolveLinksFromHtml(html,{source,year:2026,module:'I',day:1,area:null});assert.match(one.exam,/m1\.pdf$/);assert.match(one.answerKey,/g1\.pdf$/);const three=resolveLinksFromHtml(html,{source,year:2026,module:'III',day:1,area:'Economia e Administração'});assert.match(three.exam,/m3\.pdf$/)});
...
test('consulta à COPESE tem repetição automática e tempo limite',()=>{const api=read('api/pism-official.mjs');assert.match(api,/SOURCE_ATTEMPTS=2/);assert.match(api,/setTimeout\(\(\)=>controller\.abort\(\),12000\)/);assert.match(api,/response\.status<500/)});
...
test('PISM usa leitor interno com proxy oficial da UFJF',()=>{const reader=read('js/enem-document-v32.js'),pkg=read('package.json');assert.match(reader,/V29_KEY='gplus_pism_official_session_v29'/);assert.match(reader,/kind==='v29'\?'pism':'enem'/);assert.match(reader,/#v29PismOfficialRunner \.v29-paper iframe/);assert.match(reader,/GABARITO_OFFICIAL_DOCUMENT/);assert.match(pkg,/api\/pism-pdf\.mjs/)});
