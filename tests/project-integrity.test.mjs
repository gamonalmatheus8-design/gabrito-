...
test('arquivos essenciais da release comercial existem',()=>{
  for(const f of [
    ...
    'js/enem-simulator-v24.js','assets/enem-simulator-v24.css','js/pism-simulator-v25.js','assets/pism-simulator-v25.css','data/pism-questions-v25.js','data/pism-discursives-v25.js',
...
test('versões técnicas estão alinhadas em 2.6.0',()=>{const pkg=JSON.parse(read('package.json'));const boot=read('js/gabarito-bootstrap.js');const sw=read('service-worker.js');assert.equal(pkg.version,'2.6.0');assert.match(boot,/const VERSION='2\.6\.0'/);assert.match(sw,/gabarito-mais-2-6-0-app-shell/);assert.match(sw,/const V='2\.6\.0'/)});
