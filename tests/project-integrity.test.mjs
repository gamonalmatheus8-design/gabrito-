...
test('arquivos essenciais da release comercial existem',()=>{
  for(const f of [
    'supabase/schema.sql','js/supabase-config.js','js/gabarito-bootstrap.js','js/gabarito-question-source.js','js/official-question-bank-v1.js','js/official-practice-v1.js','js/official-practice-navigation-v1.js','js/official-practice-stability-v1.js',
    ...
    'js/enem-history-v28.js','js/enem-direct-sync-v36.js','assets/enem-history-v28.css','js/pism-history-v29.js','js/pism-direct-sync-v37.js','assets/pism-history-v29.css','data/pism-questions-v25.js','data/pism-discursives-v25.js',
...
test('atalhos de Questões abrem somente o treino oficial',()=>{
  const app=read('js/app.js'),nav=read('js/official-practice-navigation-v1.js'),practice=read('js/official-practice-v1.js'),stability=read('js/official-practice-stability-v1.js');
  assert.match(app,/window\.v40OpenFocusedQuestions/);
  assert.match(nav,/official-practice-stability-v1\.js\?v=2\.0\.0-20260908/);
  assert.match(nav,/window\.v40OpenFocusedQuestions=\(\)=>window\.GABARITO_OFFICIAL_PRACTICE\?\.open\?\.\(\)/);
  assert.match(nav,/window\.v42OpenQuestions=\(\)=>window\.GABARITO_OFFICIAL_PRACTICE\?\.open\?\.\(\)/);
  assert.match(stability,/if\(page==='questions'\)return openPractice\(\)/);
  assert.match(stability,/mocks\?\.classList\.remove\('active'\)/);
  assert.match(stability,/interceptQuestionClick/);
  assert.match(stability,/questionRouteOwner='official-practice-v2'/);
  assert.match(practice,/baseGo\('questions'\)/);
});

test('handlers inline principais possuem implementação',()=>{const html=read('index.html');const js=['app.js','v6-release.js','gabarito-ui.js','gabarito-supabase.js','gabarito-bootstrap.js','commercial-v2.js','official-question-bank-v1.js','official-practice-v1.js','official-practice-navigation-v1.js','official-practice-stability-v1.js','official-simulators-host.js','lazy-simulators-v32.js','enem-official-v27.js','enem-history-v28.js','enem-native-v31.js','enem-native-integration-v31.js','enem-document-v32.js','enem-mobile-v30.js','pism-history-v29.js','pism-direct-sync-v37.js'].map(f=>read('js/'+f)).join('\n');const names=[...html.matchAll(/on(?:click|submit|change|input)="\s*([A-Za-z_$][\w$]*)\s*\(/g)].map(m=>m[1]);const missing=[...new Set(names)].filter(name=>!new RegExp(`(?:function\\s+${name}\\s*\\(|window\\.${name}\\s*=|window\\[.{0,20}${name})`).test(js));assert.deepEqual(missing,[])});

test('release principal e PWA estão alinhados em 3.8.0',()=>{const pkg=JSON.parse(read('package.json'));const html=read('index.html');const boot=read('js/gabarito-bootstrap.js');const sw=read('service-worker.js');const release=read('js/v6-release.js');const lazy=read('js/lazy-simulators-v32.js');assert.equal(pkg.version,'3.8.0');assert.match(html,/gabarito-bootstrap\.js\?v=/);assert.match(boot,/const VERSION='3\.8\.0'/);assert.match(sw,/gabarito-mais-3-8-0-question-route-v2/);assert.match(sw,/const V='3\.8\.0'/);assert.match(sw,/20260908-question-route-v2/);assert.match(release,/SW_VERSION='3\.8\.0'/);assert.match(release,/SW_RECOVERY='20260908-question-route-v2'/);assert.match(lazy,/simulatorReader='official-direct-sync'/);assert.match(lazy,/enemSimulatorReader='direct-sync-v36'/);assert.match(lazy,/pismSimulatorReader='direct-sync-v37'/)});

test('Banco Oficial alimenta um modo treino separado de Simulados',()=>{const boot=read('js/gabarito-bootstrap.js'),bank=read('js/official-question-bank-v1.js'),practice=read('js/official-practice-v1.js'),nav=read('js/official-practice-navigation-v1.js'),stability=read('js/official-practice-stability-v1.js');assert.match(boot,/js\/official-question-bank-v1\.js/);assert.match(boot,/js\/official-practice-v1\.js/);assert.match(boot,/js\/official-practice-navigation-v1\.js/);assert.ok(boot.indexOf('js/official-question-bank-v1.js')<boot.indexOf('js/official-practice-v1.js'));assert.match(bank,/official_question_index/);assert.match(bank,/questionPracticeSource='official-exams-only'/);assert.match(practice,/questionPracticeMode='official-single-question'/);assert.match(practice,/baseGo\('questions'\)/);assert.match(nav,/questionPracticeSeparatedFromMocks=true/);assert.match(stability,/questionPracticeSeparatedFromMocks=true/);assert.match(stability,/mocks\?\.classList\.remove\('active'\)/);assert.match(practice,/authorialQuestionPractice=false/);assert.doesNotMatch(practice,/#v28HistoryLibrary|#v29PismOfficialHub/)});

test('treino oficial registra acerto e erro e calcula diagnóstico sem cronômetro de simulado',()=>{const practice=read('js/official-practice-v1.js');assert.match(practice,/official_practice_attempts/);assert.match(practice,/is_correct:isCorrect/);assert.match(practice,/function diagnosisGroups/);assert.match(practice,/Aproveitamento/);assert.match(practice,/Meus erros/);assert.match(practice,/Não respondidas/);assert.doesNotMatch(practice,/setInterval\(/)});

test('PISM no treino usa caderno e gabarito oficiais da COPESE para feedback',()=>{const practice=read('js/official-practice-v1.js');assert.match(practice,/\/api\/pism-official\?/);assert.match(practice,/kind=\$\{kind\}/);assert.match(practice,/function parsePismKeyText/);assert.match(practice,/resolvePism\('key',q\)/);assert.match(practice,/\/api\/pism-pdf\?url=/)});

test('ENEM oficial substitui o simulador legado e carrega sob demanda',()=>{const boot=read('js/gabarito-bootstrap.js'),lazy=read('js/lazy-simulators-v32.js'),official=read('js/enem-official-v27.js');assert.doesNotMatch(boot,/js\/enem-official-v27\.js/);assert.match(lazy,/s\.async=true/);assert.match(lazy,/js\/enem-official-v27\.js/);assert.match(lazy,/js\/enem-history-v28\.js/);assert.match(lazy,/js\/enem-direct-sync-v36\.js/);assert.match(official,/minutes:330/);assert.match(official,/minutes:300/);assert.match(official,/O ENEM calcula a nota com TRI/)});

test('PISM oficial substitui o simulador legado e usa leitor sincronizado',()=>{const boot=read('js/gabarito-bootstrap.js'),lazy=read('js/lazy-simulators-v32.js'),official=read('js/pism-history-v29.js'),sync=read('js/pism-direct-sync-v37.js');assert.doesNotMatch(boot,/js\/pism-history-v29\.js/);assert.match(lazy,/data\/pism-official-catalog-v29\.js/);assert.match(lazy,/assets\/pism-history-v29\.css/);assert.match(lazy,/js\/pism-history-v29\.js/);assert.match(lazy,/js\/pism-direct-sync-v37\.js/);assert.match(official,/minutes\|\|240/);assert.match(official,/20 marcadas/);assert.match(official,/fonte oficial UFJF/);assert.match(sync,/\/api\/pism-pdf\?url=/);assert.match(sync,/GABARITO_PISM_DIRECT_SYNC/)});
...
test('boot mantém fallback interno sem expô-lo como prática de Questões',()=>{const boot=read('js/gabarito-bootstrap.js'),bank=read('js/official-question-bank-v1.js'),practice=read('js/official-practice-v1.js');assert.match(boot,/await loadLocalBank\(\)/);assert.match(boot,/data\/editorial-exclusions\.js/);assert.match(boot,/bankSource='legacy-internal'/);assert.match(bank,/questionPracticeSource='official-exams-only'/);assert.match(practice,/authorialQuestionPractice=false/);assert.match(boot,/loadQualityLayer/);assert.match(boot,/Camada de qualidade indisponível/)});
