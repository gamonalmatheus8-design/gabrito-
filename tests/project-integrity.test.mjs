...
test('arquivos essenciais da release comercial existem',()=>{
  for(const f of [
    ...
    'js/enem-simulator-v24.js','assets/enem-simulator-v24.css','js/enem-official-v27.js','assets/enem-official-v27.css','js/pism-simulator-v25.js','assets/pism-simulator-v25.css','data/pism-questions-v25.js','data/pism-discursives-v25.js',
...
test('handlers inline principais possuem implementação',()=>{const html=read('index.html');const js=['app.js','v6-release.js','gabarito-ui.js','gabarito-supabase.js','gabarito-bootstrap.js','commercial-v2.js','enem-simulator-v24.js','enem-official-v27.js','pism-simulator-v25.js'].map(f=>read('js/'+f)).join('\n');const names=[...html.matchAll(/on(?:click|submit|change|input)="\s*([A-Za-z_$][\w$]*)\s*\(/g)].map(m=>m[1]);const missing=[...new Set(names)].filter(name=>!new RegExp(`(?:function\\s+${name}\\s*\\(|window\\.${name}\\s*=|window\\[.{0,20}${name})`).test(js));assert.deepEqual(missing,[])});

test('versões técnicas estão alinhadas em 3.0.1',()=>{const pkg=JSON.parse(read('package.json'));const html=read('index.html');const boot=read('js/gabarito-bootstrap.js');const sw=read('service-worker.js');assert.equal(pkg.version,'3.0.1');assert.match(html,/gabarito-bootstrap\.js\?v=3\.0\.1&r=20260901h/);assert.match(boot,/const VERSION='3\.0\.1'/);assert.match(sw,/gabarito-mais-3-0-1-app-shell/);assert.match(sw,/const V='3\.0\.1'/)});

test('simulador ENEM autoral replica os dois dias e mantém resultado bruto sem falsa TRI',()=>{const s=read('js/enem-simulator-v24.js'),boot=read('js/gabarito-bootstrap.js');assert.match(s,/DAY_MINUTES=\{1:330,2:300\}/);assert.match(s,/DAY_AREAS=\{1:\['Linguagens','Ciências Humanas'\],2:\['Ciências da Natureza','Matemática'\]\}/);assert.match(s,/buildLinguagens/);assert.match(s,/45/);assert.match(s,/Redação/);assert.match(s,/Rascunho/);assert.match(s,/Folha final/);assert.match(s,/cartão-resposta/i);assert.match(s,/não são convertidos aqui em uma nota oficial/i);assert.match(boot,/assets\/enem-simulator-v24\.css/);assert.match(boot,/js\/enem-simulator-v24\.js/)});

test('simulador PISM replica módulos, dois dias, discursivas e pontuação do edital 2027',()=>{const s=read('js/pism-simulator-v25.js'),boot=read('js/gabarito-bootstrap.js'),extra=read('data/pism-discursives-v25.js'),extraQ=read('data/pism-questions-v25.js');assert.match(s,/const MINUTES=240/);assert.match(s,/I:\{1:\['Português','Geografia','Matemática','Química'\],2:\['Literatura','Biologia','Física','História'\]\}/);assert.match(s,/III:\{1:\['Português','Literatura','Biologia','Matemática'\],2:\['Física','Química','Geografia','História'\]\}/);for(const area of ['Economia e Administração','Exatas','Humanas','Saúde'])assert.match(s,new RegExp(area));assert.match(s,/'Português':4,'Matemática':4,'Sociologia':2/);assert.match(s,/'Geografia':4,'História':4,'Filosofia':2/);assert.match(s,/'Português':5,'Matemática':5/);assert.match(s,/'Física':5,'Química':5/);assert.match(s,/20 objetivas/);assert.match(s,/5 pontos cada/);assert.match(s,/autocorreção/i);assert.match(s,/pontuação oficial depende da banca da UFJF/i);assert.match(extra,/P25D3-PORT-05/);assert.match(extra,/P25D3-QUI-05/);assert.match(extraQ,/P25QI-PORT-01/);assert.match(extraQ,/P25QIII-HIS-01/);assert.match(boot,/data\/pism-questions-v25\.js/);assert.match(boot,/data\/pism-discursives-v25\.js/);assert.match(boot,/assets\/pism-simulator-v25\.css/);assert.match(boot,/js\/pism-simulator-v25\.js/)});
