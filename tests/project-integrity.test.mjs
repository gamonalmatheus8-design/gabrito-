...
test('arquivos essenciais da release comercial existem',()=>{
  for(const f of [
    'supabase/schema.sql','js/supabase-config.js','js/gabarito-bootstrap.js','js/gabarito-question-source.js','js/official-question-bank-v1.js',
...
test('atalho de questões é redirecionado para o banco oficial',()=>{const app=read('js/app.js'),bank=read('js/official-question-bank-v1.js');assert.match(app,/window\.v40OpenFocusedQuestions/);assert.match(bank,/window\.v40OpenFocusedQuestions=\(\)=>openBank\(\)/);assert.match(bank,/window\.v42OpenQuestions=\(\)=>openBank\(\)/)});

test('handlers inline principais possuem implementação',()=>{const html=read('index.html');const js=['app.js','v6-release.js','gabarito-ui.js','gabarito-supabase.js','gabarito-bootstrap.js','commercial-v2.js','official-question-bank-v1.js','official-simulators-host.js','lazy-simulators-v32.js','enem-official-v27.js','enem-history-v28.js','enem-native-v31.js','enem-native-integration-v31.js','enem-document-v32.js','enem-mobile-v30.js','pism-history-v29.js','pism-direct-sync-v37.js'].map(f=>read('js/'+f)).join('\n');const names=[...html.matchAll(/on(?:click|submit|change|input)="\s*([A-Za-z_$][\w$]*)\s*\(/g)].map(m=>m[1]);const missing=[...new Set(names)].filter(name=>!new RegExp(`(?:function\\s+${name}\\s*\\(|window\\.${name}\\s*=|window\\[.{0,20}${name})`).test(js));assert.deepEqual(missing,[])});

test('release principal e PWA estão alinhados em 3.7.0',()=>{const pkg=JSON.parse(read('package.json'));const html=read('index.html');const boot=read('js/gabarito-bootstrap.js');const sw=read('service-worker.js');const release=read('js/v6-release.js');const lazy=read('js/lazy-simulators-v32.js');assert.equal(pkg.version,'3.7.0');assert.match(html,/gabarito-bootstrap\.js\?v=/);assert.match(boot,/const VERSION='3\.7\.0'/);assert.match(boot,/20260908-official-bank-v2/);assert.match(sw,/gabarito-mais-3-7-0-official-bank/);assert.match(sw,/const V='3\.7\.0'/);assert.match(release,/SW_VERSION='3\.7\.0'/);assert.match(lazy,/const VERSION='3\.7\.0'/);assert.match(lazy,/simulatorReader='official-direct-sync'/);assert.match(lazy,/enemSimulatorReader='direct-sync-v36'/);assert.match(lazy,/pismSimulatorReader='direct-sync-v37'/)});

test('Banco Oficial substitui a prática autoral na navegação de Questões',()=>{const boot=read('js/gabarito-bootstrap.js'),bank=read('js/official-question-bank-v1.js');assert.match(boot,/js\/official-question-bank-v1\.js/);assert.match(bank,/if\(page==='questions'\)return openBank\(\)/);assert.match(bank,/questionPracticeSource='official-exams-only'/);assert.match(bank,/authorialQuestionPractice=false/);assert.match(bank,/#v28HistoryLibrary/);assert.match(bank,/#v29PismOfficialHub/)});
