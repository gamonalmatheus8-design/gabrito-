...
test('atalhos de Questões usam a navegação nativa e não abrem Simulados',()=>{
  ...
  assert.match(practice,/GABARITO_PRACTICE_V3\?\.open/);
  assert.doesNotMatch(nav,/window\.go\s*=/);
  assert.doesNotMatch(stability,/MutationObserver|setInterval|interceptQuestionClick/);
});
...
test('release principal e PWA estão alinhados em 3.9.0',()=>{const pkg=JSON.parse(read('package.json'));const html=read('index.html');const boot=read('js/gabarito-bootstrap.js');const sw=read('service-worker.js');const release=read('js/v6-release.js');const lazy=read('js/lazy-simulators-v32.js');assert.equal(pkg.version,'3.9.0');assert.match(html,/gabarito-bootstrap\.js\?v=/);assert.match(boot,/const VERSION='3\.9\.0'/);assert.match(boot,/RECOVERY='20260909-practice-v3'/);assert.match(sw,/gabarito-mais-3-9-0-practice-v3/);assert.match(sw,/const V='3\.9\.0'/);assert.match(sw,/20260909-practice-v3/);assert.match(release,/SW_VERSION='3\.9\.0'/);assert.match(release,/SW_RECOVERY='20260909-practice-v3'/);assert.match(lazy,/simulatorReader='official-direct-sync'/);assert.match(lazy,/enemSimulatorReader='direct-sync-v36'/);assert.match(lazy,/pismSimulatorReader='direct-sync-v37'/)});

test('Banco V3 alimenta treino individual separado de Simulados',()=>{const boot=read('js/gabarito-bootstrap.js'),bank=read('js/official-question-bank-v1.js'),practice=read('js/official-practice-v1.js'),nav=read('js/official-practice-navigation-v1.js'),stability=read('js/official-practice-stability-v1.js');assert.match(boot,/js\/official-question-bank-v1\.js/);assert.match(boot,/js\/official-practice-v1\.js/);assert.match(boot,/js\/official-practice-navigation-v1\.js/);assert.ok(boot.indexOf('js/official-question-bank-v1.js')<boot.indexOf('js/official-practice-v1.js'));assert.match(bank,/practice_questions/);assert.match(bank,/practice_question_sources/);assert.match(bank,/status:'eq\.published'/);assert.match(bank,/questionBankMode='validated_practice_v3'/);assert.match(practice,/GABARITO_PRACTICE_V3/);assert.match(nav,/questionPracticeSeparatedFromMocks=true/);assert.match(stability,/questionPracticeSeparatedFromMocks=true/);assert.match(bank,/authorialQuestionPractice=false/);assert.doesNotMatch(bank,/official_question_index|#v28HistoryLibrary|#v29PismOfficialHub/);assert.doesNotMatch(bank,/go\(['"]mocks['"]\)/)});

test('Banco V3 registra acerto e erro e calcula diagnóstico sem cronômetro de simulado',()=>{const bank=read('js/official-question-bank-v1.js');assert.match(bank,/practice_attempts/);assert.match(bank,/is_correct:ok/);assert.match(bank,/renderDiagnosis/);assert.match(bank,/Aproveitamento/);assert.match(bank,/Meus erros/);assert.match(bank,/Não respondidas/);assert.doesNotMatch(bank,/setInterval\(/)});

test('PISM só entra no treino após validação editorial individual',()=>{const bank=read('js/official-question-bank-v1.js');assert.match(bank,/PISM em validação editorial/);assert.match(bank,/\/api\/pism-pdf\?url=/);assert.match(bank,/Somente itens publicados após validação de fonte e gabarito/);assert.doesNotMatch(bank,/\/api\/pism-official\?/)});
...
test('boot mantém fallback interno sem expô-lo como prática de Questões',()=>{const boot=read('js/gabarito-bootstrap.js'),bank=read('js/official-question-bank-v1.js');assert.match(boot,/await loadLocalBank\(\)/);assert.match(boot,/data\/editorial-exclusions\.js/);assert.match(boot,/bankSource='legacy-internal'/);assert.match(bank,/questionBankMode='validated_practice_v3'/);assert.match(bank,/authorialQuestionPractice=false/);assert.doesNotMatch(bank,/ENEM_QUESTIONS|PISM_QUESTIONS/);assert.match(boot,/loadQualityLayer/);assert.match(boot,/Camada de qualidade indisponível/)});
