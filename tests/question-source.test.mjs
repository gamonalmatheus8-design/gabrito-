...
function clientFor({fail=false,partial=false}={}){
  const tables={questions:dbRows(),pism_discursives:discursives.map(x=>({...x,editorial_status:'publicada'})),app_meta:[{key:'question_bank_version',value:'1.1.0'},{key:'autoral_question_count',value:String(questions.length)}]};
  if(partial)tables.questions=tables.questions.slice(0,100);
...
test('carrega Supabase paginado e converte os campos editoriais',async()=>{
  ...
  assert.equal(result.questions.filter(x=>x.exam==='ENEM').length,questions.filter(q=>q.exam==='ENEM').length);
  assert.equal(result.questions.filter(x=>x.exam==='PISM').length,questions.filter(q=>q.exam==='PISM').length);
